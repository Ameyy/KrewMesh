...
function bestGuessBrowserZoom() {
  ...
  const ratio = (window.outerWidth || innerWidth) / innerWidth;
...
function useMergeRefs<T>(refs: Array<React.Ref<T> | undefined | null>) {
  const cleanupRef = useRef<(() => void) | void>(void 0);
  const refEffect = useCallback((instance: T | null) => {
...
const ShaderMount2 = forwardRef<HTMLDivElement, ShaderMountProps>(
...
function colorPropsAreEqual(prevProps: any, nextProps: any) {
  for (const key in prevProps) {
    if (key === "colors") {
      ...
      if (!prevProps.colors?.every((color: any, index: number) => color === nextProps.colors?.[index])) {
        return false;
      }
      continue;
    }
...
interface ImageDitheringProps extends React.HTMLAttributes<HTMLDivElement> {
  speed?: number;
  frame?: number;
  colorFront?: any;
  colorBack?: any;
...
function gradientDataUrl(g: typeof TICKET_GRADIENT, aspect: number) {
...
export interface AdmitOneTicketProps extends TicketCardProps {
  tilt?: boolean | Partial<TiltCardProps>;
}
...
function burst(ctx: AudioContext, at: number, opts: { gain: number; decay: number; frequency: number; q: number }) {
