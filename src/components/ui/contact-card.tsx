...
function ContactInfo({
	...
	const content = (
		<div className={cn('flex items-center gap-3 py-3 group', className)} {...props}>
			<div className="bg-muted/40 rounded-lg p-3 group-hover:bg-muted/60 transition-colors">
				<Icon className="h-5 w-5" />
			</div>
			<div>
				<p className="font-medium">{label}</p>
				<p className={cn("text-muted-foreground text-xs", href && "group-hover:text-foreground transition-colors")}>{value}</p>
			</div>
		</div>
	);
