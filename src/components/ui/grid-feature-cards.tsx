...
export function FeatureCard({ feature, className, ...props }: FeatureCardPorps) {
	...
	return (
		<div className={cn('relative overflow-hidden p-8', className)} {...props}>
			...
			<h3 className="mt-12 text-lg md:text-2xl uppercase tracking-wider font-semibold">{feature.title}</h3>
			<p className="text-muted-foreground relative z-20 mt-4 text-sm font-light">{feature.description}</p>
		</div>
	);
}
